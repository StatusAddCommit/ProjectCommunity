...
const generalJs = () => 
  jQuery(function($) {
    ...
    $(function() {
      objectFitImages();
    }); // Object Fit PolyFill for IE and Safari
    ...
    window.fly = {
      ...
      scrollToAnchors: function() {
        $('.anchor[href^="#"]').on('click', function(e) {
          ...
          var speed = 1,
            ...
            targetPos =
              target != '#' && $(target).length == 1
                ? parseInt($(target).offset().top, 10) - offset
                : currPos,
          ...
          $('html, body').animate(
            { scrollTop: targetPos },
            parseInt(Math.abs(distance / (speed + boost)), 10)
          );
        });
      ...
      scrollBarWidthDetection: function() {
        ...
        scrollBarWidth =
          scrollBarDetect.width() - scrollBarDetect.find('span').width();
      ...
      pageLoader: function(isActive, maxLoadTime) {
        ...
        function hideLoader() {
          ...
          if (flySlider.length) {
            hideLoaderDelay = flySlider.data('rotation-duration')
              ? flySlider.data('rotation-duration')
              : 1000;
          }
        ...
        navItems.hover(
          ...
          function() {
            if (screenWidth > breakPoint) {
              ...
              if (Modernizr.cssanimations) {
                dropdown
                  .removeClass(animationIn + ' animated hidden')
                  .addClass(animationOut + ' animated')
                  .one(
                    'webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend',
                    function() {
                      dropdown
                        .removeClass(animationOut + ' animated')
                        .addClass('hidden');
                      dropdown.off(
                        'webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend'
                      );
      ...
      stickyMenuInit: function() {
        $.fn.stickyMenu = function() {
          var stickyMenu = $(this),
            ...
            placeholder = $('<div/>'),
            isPlaceholder = stickyMenu.is('[data-no-placeholder]')
              ? false
              : true,
        ...
        $window.on('pageLoaded', function() {
          $('.sidebar-sticky').each(function() {
            var sidebar = $(this),
              ...
              sidebarOffset = stickyMenu.length
                ? stickyMenu.outerHeight() + 20
                : 20;
      ...
      mainSlider: function() {
        $.fn.mainSliderApi = function() {
          ...
          slider
            .find('[data-animate-in], [data-animate-out]')
            .addClass('animated');
        ...
        mainSlider
          .carousel({ interval: mainSlider.data('interval'), pause: 'none' })
          .mainSliderApi();
      },
      ...
      flySliderInit: function() {
        $.fn.flySlider = function() {
          ...
          var flySlider = $('.fly-slider'),
            ...
            rotationDuration = flySlider.data('rotation-duration')
              ? flySlider.data('rotation-duration')
              : 1000,
            autoInterval = flySlider.data('rotation-interval')
              ? flySlider.data('rotation-interval')
              : 10000,
          ...
          slidesContent.css({
            transition:
              'transform ' +
              (rotationDuration / 1000 + slidesContentRotationDelay) +
              's ease'
          });
              ...
              slides.each(function(index) {
                $(this).css({
                  transform:
                    ...
                    (index * 360) / slidesQt +
                    'deg) translate3d(0, 0, ' +
                    parseInt((slideOffset * screenHeight) / 100) +
                    'px)',
      ...
      parallaxInit: function() {
        $.fn.parallax = function() {
          var parallax = $(this),
            xPos = parallax.data('parallax-position')
              ? parallax.data('parallax-position')
              : 'center',
            speed =
              parallax.data('parallax-speed') ||
              parallax.data('parallax-speed') == 0
                ? parallax.data('parallax-speed')
                : 0.1;
          ...
          function runParallax() {
            ...
            if (
              scrollTop + screenHeight > offsetTop &&
              offsetTop + parallaxHeight > scrollTop
            ) {
      ...
      statsCounter: function(infinite) {
        $('.js-counter').each(function() {
          var counter = $(this),
            ...
            countStep = counter.data('step') ? counter.data('step') : 0.1,
            count = 0,
            counting = false,
            done = false;
          ...
          function countSkills() {
            ...
            var interval = setInterval(function() {
              count = count + 1;
              counter.text(
                parseInt((countTo * count * countStep) / countTime, 10)
              );
      ...
      progressCalc: function() {
        $('.fly-project').each(function() {
          ...
          $window.on('scroll resized', function() {
            ...
            if (self.offset().top < $window.scrollTop() + screenHeight * 0.6) {
              ...
              for (var i = 1; i < 21; i++) {
                var timeOuted = function(i) {
                  return setTimeout(function() {
                    progressLabel
                      .css({ left: (percent * i) / 20 + '%' })
                      .text(parseInt((percent * i) / 20) + '%');
